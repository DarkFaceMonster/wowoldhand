...
function Oldhand_BU(s) local P,B,i="player",true,1  while UnitBuff(P,i)   do if string.find(UnitBuff(P,i),s)   then B=false end i=i+1 end return B end
...
function Oldhand_GetSpellCooldown(spellname) 
	i = Oldhand_GetActionID(spellname);
...
function Oldhand_TestPlayerIsHorse()
  ...
  if name ~= null then
	...
	while UnitBuff("player", i ) do
		...
		local	vText1 = OldhandTooltipTextLeft1:GetText();
		local	vText2 = OldhandTooltipTextLeft2:GetText();		
		OldhandTooltip:Hide();
		if vText2 ~= nil then
...
function Oldhand_TestHasBoss()	
	if (not UnitIsDeadOrGhost("player"))  and UnitExists("playertarget") then		    
		if(not UnitIsFriend("player","target")) and (not UnitIsDead("playertarget")) then			
			for k, v in pairs(Oldhand_Bosses) do							
...
function Oldhand_No_Sneer_Boss()	
	if (not UnitIsDeadOrGhost("player"))  and UnitExists("playertarget") then		    
		if(not UnitIsFriend("player","target")) and (not UnitIsDead("playertarget")) then			
			for k, v in pairs(Oldhand_No_Sneer_Bosses) do							
				if(UnitName("playertarget") == v) then
					return true;						
				end		
			end
...
function Oldhand_ImmuneSpell(spellname)
	if (not UnitIsDeadOrGhost("player"))  and UnitExists("playertarget") then		    
		if(not UnitIsFriend("player","target")) and (not UnitIsDead("playertarget")) then
			for k, v in pairs(Oldhand_SaveData) do		       
...
function Oldhand_TestSelfOT()	
	if (not UnitIsDeadOrGhost("player"))  and UnitExists("playertarget") then		    
		if(not UnitIsFriend("player","target")) and (not UnitIsDead("playertarget")) then			
			for k, v in pairs(Oldhand_OT_Bosses) do							
				if(UnitName("playertarget") == v) then						
...
function Oldhand_Test_Target_IsMe()	
	if UnitExists("playertargettarget") then	
		if UnitIsUnit("playertargettarget", "player") then
			return true;	
		end
...
function Oldhand_Test_Target_Debuff()
	if  UnitExists("playertarget") then		    
			if UnitCanAttack("player","target") and (not UnitIsDead("playertarget")) then
				if not Oldhand_TargetDeBU("媚惑")  
					or not Oldhand_TargetDeBU("变形术") 
					or not Oldhand_TargetDeBU("休眠")  
					or not Oldhand_TargetDeBU("致盲")  		
					or not Oldhand_TargetDeBU("闷棍") 				
					or not Oldhand_TargetDeBU("冰冻陷阱")
					or not Oldhand_TargetDeBU("忏悔")
				then
					return true;
				end
			end
...
function Oldhand_TestTrinket(TrinketName)
	if Oldhand_GetItemInfo(13) == TrinketName or Oldhand_GetItemInfo(14) == TrinketName then
		return true;
	end
	return false;
end
function Oldhand_GetMainHandType()
...
function Oldhand_GetAssistantHandType()
...
function Oldhand_GetUnitAssistantHandType(unit)
...
function Oldhand_Warning_AddMessage(str)
	if Messagestr ~= str then
		Messagestr = str;
		if Oldhand_Test_Battlefield() then
			DEFAULT_CHAT_FRAME:AddMessage("|cffffff00战斗信息:|r |cff00ff00" .. str .. "|r");	
		else
...
function Oldhand_Warning_AddMessage1(str)
	if Oldhand_Test_Battlefield() then
		DEFAULT_CHAT_FRAME:AddMessage("|cffffff00战斗信息:|r |cff00ff00" .. str .. "|r");	
	else
		if (UnitInRaid("player")) then
...
function Oldhand_Test_Battlefield()
...
function Oldhand_AddMessage_A(str)
		DEFAULT_CHAT_FRAME:AddMessage("|cffffff00战斗信息:|r |cff00ff00" .. str .. "|r");	
end

function Oldhand_SetText(str,rgb)
	Oldhand_MSG_Text:SetText(str..", "..rgb);		
	getglobal("OldhandColorRectangle".."NormalTexture"):SetVertexColor((rgb - rgb % 10)/100 , 0, (rgb % 10)/10);	
end

function  Oldhand_Auto_Attack()
	for i = 1, 120 do
		if ( HasAction(i) ) then
...
function Oldhand_GetActionIDbyName(text)
	for i = 1, 120 do
		if ( HasAction(i) ) then
...
-- 不考虑距离
function Oldhand_CastSpell_IgnoreRange(text, texture)
	for i = 61, 72 do
		if ( HasAction(i) ) then
			local temptexture = GetActionTexture(i);
			if temptexture == texture then
...
function Oldhand_CastSpellbyID(text,texture,i)
	if i > 0 then
		if ( HasAction(i) ) then
			if Oldhand_CanUseAction(i) and (0~=IsActionInRange(i)) then
				local index = i;
				if (i >= 61 and i<=72) then
					index = i - 48;
				end
...
function Oldhand_PickupSpellByBook(spell)
	local i = 1
	while true do
