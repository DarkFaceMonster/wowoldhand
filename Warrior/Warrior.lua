...
local   Warrior_DISEASE = '疾病';
local 	Warrior_MAGIC   = '魔法';
local 	Warrior_POISON  = '中毒';
local 	Warrior_CURSE   = '诅咒';

action_table = {};

action_table["自动攻击"] = 132400;

action_table["冲锋"] = 132337;
action_table["嘲讽"] = 136080;

-- 狂怒
action_table["嗜血"] = 136012;
action_table["狂暴挥砍"] = 132367;
action_table["怒击"] = 132352;
action_table["斩杀"] = 135358;
action_table["暴怒"] = 132352;
action_table["狂怒回复"] = 132345;
action_table["拳击"] = 132938;

-- 防护
action_table["毁灭打击"] = 135291;
action_table["盾牌猛击"] = 134951;
action_table["雷霆一击"] = 136105;
action_table["乘胜追击"] = 132342;
action_table["盾牌格挡"] = 132110;
-- 饰品
action_table["临近风暴之怒"] = 236164;
action_table["伊萨诺斯甲虫"] = 236164;
local spell_table = {};
...
function Warrior_UnitPopup_OnClick()
	local index = this.value;
	local dropdownFrame = getglobal(UIDROPDOWNMENU_INIT_MENU);
	local button =  this.value;	
	local name = dropdownFrame.name;

	if ( button == "WarriorPOPUP" ) then
		Warrior_AutoFollowName = name;
		PlaySound("UChatScrollButton");
		DEFAULT_CHAT_FRAME:AddMessage("("..Warrior_AutoFollowName..")目前为跟随对象!");
	else
		Warrior_Old_UnitPopup_OnClick();
	end
end
			    ...
			    if 0==1 and  Talents == MaxTalents then
				   ...
				   if not UnitCanAttack("player","target")  then
						if UnitClass("target") == "德鲁伊" and GetTalentTabInfo(s,true) == "野性战斗" then
							Warrior_AddPlayerTalent(UnitName("target"));
						end;
...
function Warrior_AddPlayerTalent(playername)
	for k, v in pairs(Warrior_PlayerTalentInfoDatas) do							
		if(playername == v["Name"]) then
			return;
		end		
	end

	table.insert(Warrior_PlayerTalentInfoDatas,
		{
		["Name"] = playername,
		});
end
function Warrior_GetPlayerTalent(playername)
	for k, v in pairs(Warrior_PlayerTalentInfoDatas) do							
		if(playername == v["Name"]) then
			return true;
		end		
...
function  Warrior_PlayerVsPlayer()
	return;
end;
...
function Warrior_Auto_Trinket()
	if not Warrior_PlayerBU("水手的迅捷") then
		if Warrior_TestTrinket("大副的怀表") and (IsActionInRange(Warrior_GetActionID("Ability_Warrior_Lavalash")) ~= 0)  then
			if Warrior_CastSpell("大副的怀表","INV_Misc_PocketWatch_02") then return true; end		
		end
	end
	if not Warrior_PlayerBU("激烈怒火") then
		if Warrior_TestTrinket("临近风暴之怒") and (IsActionInRange(Warrior_GetActionID("Ability_Warrior_Lavalash")) ~= 0)  then
			if Warrior_CastSpell_IgnoreRange("临近风暴之怒", action_table["临近风暴之怒"]) then return true; end	
		end
	end
	if not Warrior_PlayerBU("银色英勇") then
		if Warrior_TestTrinket("菲斯克的怀表") and (IsActionInRange(Warrior_GetActionID("Ability_Warrior_Lavalash")) ~= 0)  then
			if Warrior_CastSpell("菲斯克的怀表","INV_Misc_AhnQirajTrinket_03") then return true; end		
		end
	end
	if not Warrior_PlayerBU("野性狂怒") then
		if Warrior_TestTrinket("战歌的热情") and (IsActionInRange(Warrior_GetActionID("Ability_Warrior_Lavalash")) ~= 0)  then
			if Warrior_CastSpell("战歌的热情","INV_Misc_Horn_02") then return true; end		
		end
	end
	if not Warrior_PlayerBU("奥术灌注") then
		if Warrior_TestTrinket("伊萨诺斯甲虫") and (IsActionInRange(Warrior_GetActionID("Ability_Warrior_Lavalash")) ~= 0)  then
			if Warrior_CastSpell_IgnoreRange("伊萨诺斯甲虫", action_table["伊萨诺斯甲虫"]) then return true; end		
		end
	end
	
	if not Warrior_PlayerBU("精准打击") then
		if Warrior_TestTrinket("苔原护符") and (IsActionInRange(Warrior_GetActionID("Ability_Warrior_Lavalash")) ~= 0)  then
			if Warrior_CastSpell("苔原护符","INV_Jewelcrafting_StarOfElune_03") then return true; end;
		end
	end
	if not Warrior_PlayerBU("凶暴") then
		if Warrior_TestTrinket("刃拳的宽容") and (IsActionInRange(Warrior_GetActionID("Ability_Warrior_Lavalash")) ~= 0)  then
			if Warrior_CastSpell("刃拳的宽容","INV_DataCrystal06") then return true; end		
		end
	end
	if not Warrior_PlayerBU("燃烧之恨") then
		if Warrior_TestTrinket("食人魔殴斗者的徽章") and (IsActionInRange(Warrior_GetActionID("Ability_Warrior_Lavalash")) ~= 0)  then
			if Warrior_CastSpell("食人魔殴斗者的徽章","INV_Jewelry_Talisman_04") then return true; end		
		end
	end
	
	--if Warrior_GetActionID("INV_Misc_MonsterScales_15") ~= 0 then
	--	if Warrior_TestTrinket("嗜血胸针") and (IsActionInRange(Warrior_GetActionID("Ability_Warrior_Lavalash")) == 1)  then
	--		if Warrior_CastSpell("嗜血胸针","INV_Misc_MonsterScales_15") then return true; end		
	--	end
	--end;
	--if  Warrior_GetActionID("INV_ValentinePerfumeBottle") ~= 0 then
	--	if Warrior_TestTrinket("殉难者精华") and (IsActionInRange(Warrior_GetActionID("Spell_Holy_HolyBolt")) == 1)  then
	--		if Warrior_CastSpell("殉难者精华","INV_ValentinePerfumeBottle") then return true; end		
	--	end
	--end  	
	return false;
end
...
function Warrior_DpsOut1()
  if Warrior_Test_Target_Debuff() then 
	...
	if 0~=IsActionInRange(Warrior_GetActionID(action_table["风剪"])) then
		local spellname = UnitCastingInfo("target") 
		if null~=spellname then
			if Warrior_CastSpell("风剪", action_table["风剪"]) then return true; end;
		end;
	end;
	
	if UnitIsPlayer("target") and UnitCanAttack("player","target") then
		if Warrior_TargetDeBU("冰霜震击") then
			if Warrior_CastSpell("冰霜震击", action_table["冰霜震击"]) then return true; end;
		end;
	end;
  ...
  local partyNum = GetNumGroupMembers();
	...
	if (partyNum >= 1 and power >= 20) or power >= 40 then
	  if Warrior_CastSpell("大地震击", action_table["大地震击"]) then return true; end;
	end;
	...
	if (not debuff1 or remainTime1 < 5) then
	  ...
	  if Warrior_CastSpell("烈焰震击", action_table["烈焰震击"]) then return true; end;
	end
	
	local buff1, remainTime1, count1 = Warrior_PlayerBU("熔岩奔腾");
	if buff1 then
	  if Warrior_CastSpell("熔岩爆裂", action_table["熔岩爆裂"]) then return true; end;
	end;
	...
	if maxHealth2 > maxHealth1 then
	  if Warrior_CastSpell_IgnoreRange("火元素", action_table["火元素"]) then return true; end;
	end;
  
	if Warrior_CastSpell_IgnoreRange("元素冲击", action_table["元素冲击"]) then return true; end;
	...
	if partyNum >= 1 then
	  if Warrior_CastSpell("闪电链", action_table["闪电链"]) then return true; end;
	else
	  if Warrior_CastSpell("闪电箭", action_table["闪电箭"]) then return true; end;
	end;
...
function Warrior_DpsOut2()
	...
	if target_health_percent <= 20 and power >= 25 then
  	if Warrior_CastSpell("斩杀", action_table["斩杀"]) then return true; end;
  end;
  
	local spellname = UnitCastingInfo("target") 
	if null~=spellname then
		if Warrior_CastSpell("拳击", action_table["拳击"]) then return true; end;
	end;
...
--    if Warrior_CastSpell("怒击", action_table["怒击"]) then return true; end;
--  end;
  ...
  if power >= 85 then
    if Warrior_CastSpell("暴怒", action_table["暴怒"]) then return true; end;
  elseif power >= 60 then
    if Warrior_CastSpell("毁灭闪电", action_table["毁灭闪电"]) then return true; end;
  elseif power >= 20 then
    if Warrior_CastSpell("风暴打击", action_table["风暴打击"]) then return true; end;
  end

  if Warrior_CastSpell("嗜血", action_table["嗜血"]) then return true; end;
  if Warrior_CastSpell("狂暴挥砍", action_table["狂暴挥砍"]) then return true; end;
...
--	end
--	if debufftype==3 or debufftype==2 then
--		if Warrior_CastSpell("净化术", action_table["净化术"]) then return true; end;
--	end
	if debufftype==1 or debufftype==4 then
		if Warrior_CastSpell("净化灵魂", action_table["净化灵魂"]) then return true; end;
	end
	...
	if HealthPercent < 60 then
		if Warrior_CastSpell("星界转移", action_table["星界转移"]) then return true; end;
	end;
	
	if Warrior_DPS == 1 then
  	if HealthPercent < 50 then
  		if Warrior_CastSpell("治疗之涌", action_table["治疗之涌"]) then return true; end;
  	end;
  elseif Warrior_DPS == 2 then
    ...
    local spell_name,_,_,count = UnitBuff("player", "漩涡武器");
  	if UnitIsPlayer("playertarget") or HealthPercent < 55 then
  		if (spell_name~=null and count>4) or UnitAffectingCombat("player")==0 then
  			if Warrior_CastSpell("治疗波", action_table["治疗波"]) then return true; end;
  		end
  	end;
  	if HealthPercent < 60 then
  		if (spell_name~=null and count>3) or UnitAffectingCombat("player")==0 then
  			if Warrior_CastSpell("次级治疗波", action_table["次级治疗波"]) then return true; end;
  		end		
  	end
