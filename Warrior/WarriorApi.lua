...
function Warrior_BU(s) local P,B,i="player",true,1  while UnitBuff(P,i)   do if string.find(UnitBuff(P,i),s)   then B=false end i=i+1 end return B end
function Warrior_TargetBU(s) local P,B,i="target",true,1  while UnitBuff(P,i)   do if string.find(UnitBuff(P,i),s)   then B=false end i=i+1 end return B end
function Warrior_TargetDeBU(s) local P,B,i="target",true,1  while UnitDebuff(P,i) do if string.find(UnitDebuff(P,i),s) then B=false end i=i+1 end return B end
function Warrior_PlayerDeBU(s)
	local name1, _,_,_,_,_,expirationTime = UnitDebuff("player", s);
	if (name1 ~= null) then 
		Warrior_AddMessage("你中了Debuff: "..name1..", 还有 "..expirationTime.." 毫秒");
		return false;
	else
		return true;
	end
...
function Warrior_UnitBU(unit,s) local P,B,i=unit,true,1  while UnitBuff(P,i)   do if string.find(UnitBuff(P,i),s)   then B=false end i=i+1 end return B end
function Warrior_UnitDeBU(unit,s) local P,B,i=unit,true,1  while UnitDebuff(P,i) do if string.find(UnitDebuff(P,i),s) then B=false end i=i+1 end return B end
function Warrior_PlayerBU(s)
	local i = 1;
...
function Warrior_GetSpellCooldown(spellname) 
	i = Warrior_GetActionID(spellname);
...
function Warrior_CanUseAction(i)
  local start, duration, enable = GetActionCooldown(i);
...
function Warrior_TestPlayerIsHorse()
	local i = 1;	
	while UnitBuff("player", i ) do
		WarriorTooltip:SetOwner(Warrior_MSG_Frame, "ANCHOR_BOTTOMRIGHT", 0, 0);
		WarriorTooltip:SetUnitBuff("player", i);		
		local	vText1 = WarriorTooltipTextLeft1:GetText();
		local	vText2 = WarriorTooltipTextLeft2:GetText();		
		WarriorTooltip:Hide();
		if vText2 ~= nil then
...
function Warrior_TestHasBoss()	
	if (not UnitIsDeadOrGhost("player"))  and UnitExists("playertarget") then		    
		if(not UnitIsFriend("player","target")) and (not UnitIsDead("playertarget")) then			
			for k, v in pairs(Warrior_Bosses) do							
...
function Warrior_No_Sneer_Boss()	
	if (not UnitIsDeadOrGhost("player"))  and UnitExists("playertarget") then		    
		if(not UnitIsFriend("player","target")) and (not UnitIsDead("playertarget")) then			
			for k, v in pairs(Warrior_No_Sneer_Bosses) do							
				if(UnitName("playertarget") == v) then
					return true;						
				end		
			end
...
function Warrior_ImmuneSpell(spellname)
	if (not UnitIsDeadOrGhost("player"))  and UnitExists("playertarget") then		    
		if(not UnitIsFriend("player","target")) and (not UnitIsDead("playertarget")) then
			for k, v in pairs(Warrior_SaveData) do		       
...
function Warrior_TestSelfOT()	
	if (not UnitIsDeadOrGhost("player"))  and UnitExists("playertarget") then		    
		if(not UnitIsFriend("player","target")) and (not UnitIsDead("playertarget")) then			
			for k, v in pairs(Warrior_OT_Bosses) do							
				if(UnitName("playertarget") == v) then						
...
function Warrior_Test_Target_IsMe()	
	if UnitExists("playertargettarget") then	
		if UnitIsUnit("playertargettarget", "player") then
			return true;	
		end
...
function Warrior_Test_Target_Debuff()
	if  UnitExists("playertarget") then		    
			if UnitCanAttack("player","target") and (not UnitIsDead("playertarget")) then
				if not Warrior_TargetDeBU("媚惑")  
					or not Warrior_TargetDeBU("变形术") 
					or not Warrior_TargetDeBU("休眠")  
					or not Warrior_TargetDeBU("致盲")  		
					or not Warrior_TargetDeBU("闷棍") 				
					or not Warrior_TargetDeBU("冰冻陷阱")
					or not Warrior_TargetDeBU("忏悔")
				then
					return true;
				end
			end
...
function Warrior_GetItemInfo(slotId)
...
function Warrior_TestTrinket(TrinketName)
	if Warrior_GetItemInfo(13) == TrinketName or Warrior_GetItemInfo(14) == TrinketName then
		return true;
	end
	return false;
end
function Warrior_GetMainHandType()
...
function Warrior_GetAssistantHandType()
...
function Warrior_GetUnitAssistantHandType(unit)
...
function Warrior_AddMessage(str)
...
function Warrior_Warning_AddMessage(str)
	if Messagestr ~= str then
		Messagestr = str;
		if Warrior_Test_Battlefield() then
			DEFAULT_CHAT_FRAME:AddMessage("|cffffff00战斗信息:|r |cff00ff00" .. str .. "|r");	
		else
...
function Warrior_Warning_AddMessage1(str)
	if Warrior_Test_Battlefield() then
		DEFAULT_CHAT_FRAME:AddMessage("|cffffff00战斗信息:|r |cff00ff00" .. str .. "|r");	
	else
		if (UnitInRaid("player")) then
...
function Warrior_Test_Battlefield()
...
function Warrior_AddMessage_A(str)
		DEFAULT_CHAT_FRAME:AddMessage("|cffffff00战斗信息:|r |cff00ff00" .. str .. "|r");	
end

function Warrior_SetText(str,rgb)
	Warrior_MSG_Text:SetText(str..", "..rgb);		
	getglobal("WarriorColorRectangle".."NormalTexture"):SetVertexColor((rgb - rgb % 10)/100 , 0, (rgb % 10)/10);	
end

function  Warrior_Auto_Attack()
	for i = 1, 120 do
		if ( HasAction(i) ) then
...
function Warrior_GetActionID(texture)
	for i = 1, 120 do
		if ( HasAction(i) ) then
...
function Warrior_GetActionIDbyName(text)
	for i = 1, 120 do
		if ( HasAction(i) ) then
...
function Warrior_CastSpell(text, texture)
	for i = 61, 72 do
		if ( HasAction(i) ) then
			local temptexture = GetActionTexture(i);
			if temptexture == texture then
				if Warrior_CanUseAction(i) and (true==IsActionInRange(i)) then
					Warrior_SetText(text,i - 48);															
					return true;
				end
			end
		end
	...
	for i = 1, 12 do
		if ( HasAction(i) ) then
			local temptexture = GetActionTexture(i);
			--Warrior_AddMessage("text: "..text..", temptexture: "..temptexture..", texture: "..texture);
			if temptexture == texture then
				if Warrior_CanUseAction(i) and (true==IsActionInRange(i)) then
					Warrior_SetText(text,i );															
					return true;
				end
			end
		end
...
-- 不考虑距离
function Warrior_CastSpell_IgnoreRange(text, texture)
	for i = 61, 72 do
		if ( HasAction(i) ) then
			local temptexture = GetActionTexture(i);
			if temptexture == texture then
				if Warrior_CanUseAction(i) then
					Warrior_SetText(text, i - 48);
					return true;
				end
			end
...
function Warrior_CastSpellById_IgnoreRange(text, i)
	if i > 0 then
		if HasAction(i) then
			if Warrior_CanUseAction(i) then
				local index = i;
...
function Warrior_CastSpellbyID(text,texture,i)
	if i > 0 then
		if ( HasAction(i) ) then
			if Warrior_CanUseAction(i) and (0~=IsActionInRange(i)) then
				local index = i;
				if (i >= 61 and i<=72) then
					index = i - 48;
				end
...
function Warrior_PickupSpellByBook(spell)
	local i = 1
	while true do
